  ...
  if($amnt > $sql1['credits'])
    {
        ...
        $sql = "UPDATE history set credits=$newCredit where id=$from";
        mysqli_query($conn,$sql);



        $newCredit = $sql2['credits'] + $amnt;
        $sql = "UPDATE history set credits=$newCredit where id=$toUser";
        mysqli_query($conn,$sql);
...
<body>
  ...
  <a class="nav-link1" href="viewusers.php">LIST OF CUSTOMER DETAILS</a>
  </li>
  </ul>
  </nav>
